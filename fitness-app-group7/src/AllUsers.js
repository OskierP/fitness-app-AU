...
export class GetAllUsers extends React.Component{

    state = {data:[]}

    componentDidMount(){
        getResponse('Users/').then(res=>{console.log(res);this.setState({data: res})})
    }
        ...
        <TableContainer component={Paper}  className="centerTable">
                <Table sx={{maxWidth:1000}}>
                    ...
                    <TableRow key={response.userId}>
                        ...
                        <TableCell>{response.accountType}</TableCell>
                    </TableRow>
                    </TableBody>)}
                </Table>
